...
const createMockEndpoint = (path, params = {}) => {
    const {
        method = 'post',
        host = 'http://mymachine:6001',
        response = 'ok'
    } = params;

    return nock(host)[method](path)
        .reply(200, response);
};
...
const createMockEndpointWithHeaders = (path, params = {}) => {
    ...
    return nock(host, { reqheaders: headers })[method](path)
        .reply(200, 'ok');
};
...
const createMockEndpointWithBody = (path, params = {}) => {
    ...
    return nock(host)[method](path, body)
        .reply(replyStatus, response);
};
